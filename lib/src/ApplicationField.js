...
export class ApplicationField {
    /** @type {any} */
    #txApplicationField;
    get txApplicationField() {
        return this.#txApplicationField;
    }
     ...
     */
    async setName(name) {
        await RequestHelper.Promise(this.#txApplicationField.setName, name, CallbackType.EmptyRequestCallback, CallbackType.ErrorCallback);
        this.#name = this.#name;
    }
    ...
    async setParameters(parameters) {
        await RequestHelper.Promise(
            this.#txApplicationField.setParameters,
            parameters,
    ...
    async setParametersObj(parameters) {
        let params = [];
        // Reihenfolge ist, name, textbefore, textfollowing, dateFormat, numberFormat
        if (!parameters.name) {
     ...
     */
    async setText(text) {
        await RequestHelper.Promise(this.#txApplicationField.setText, text, CallbackType.EmptyRequestCallback, CallbackType.ErrorCallback);
        this.#text = this.#text;
    }
    //#endregion
    ...
    async getName() {
        if (this.#name == null || typeof this.#name === 'undefined') {
            let result = await RequestHelper.Promise(this.txApplicationField.getName, CallbackType.RequestStringCallback, CallbackType.ErrorCallback);
            this.#name = result;
        }
    ...
    async getTypeName() {
        if (this.#typeName == null || typeof this.#typeName === 'undefined') {
            let result = await RequestHelper.Promise(
                this.txApplicationField.getTypeName,
    ...
    async getParameters() {
        if (this.#parameters == null || typeof this.#parameters === 'undefined') {
            let result = await RequestHelper.Promise(
                this.txApplicationField.getParameters,
    ...
    async getText() {
        if (this.#text == null || typeof this.#text === 'undefined') {
            let result = await RequestHelper.Promise(this.txApplicationField.getText, CallbackType.RequestStringCallback, CallbackType.ErrorCallback);
            this.#text = result;
        }
    ...
    async getStart() {
        if (this.#start == null || typeof this.#start === 'undefined') {
            let result = await RequestHelper.Promise(
                this.txApplicationField.getStart,
